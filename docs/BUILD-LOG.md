...
## Current state (as of 2026-04-27 — Phase 4 Day 1 Session A complete · Session B ahead)
...
- **Next milestone:** **Phase 4 Day 1 Session B — admission + scoring + verification staircase.** Inherits the locked DSL + evaluator + DealState as foundation inputs. Lands the surfaces registry literal port (rebuild plan lines 470-494), admission engine via `SurfaceAdmission.admit(...)`, scoring prompt body + tool schema, `DealIntelligence.getApplicablePatterns/Experiments/Flags` methods composing the evaluator + table reads. Possible inclusion: the parked `buildEventContext` shape-fix + backfill (decide at Session B kickoff prep — fix-first OR fix-after based on whether Session B's admission engine needs accurate event_context segmentation). See **`## Forward map`** section for downstream sequence.
...
2. **Phase 4 — Intelligence layer** (~5 days; Day 1 splits A/B per execution decision).
   - **Day 1 Session A** ✓ COMPLETE — `event_context SET NOT NULL` migration + applicability DSL + evaluator + `DealState` + `getDealState`.
   - **Day 1 Session B** (NEXT) — surfaces registry + admission engine + scoring prompt + `DealIntelligence.getApplicable*` methods + verification staircase. Possibly absorbs the parked `buildEventContext` shape-fix + backfill.
   - Day 2 — `IntelligenceCoordinator.receiveSignal` + `getActivePatterns` real implementations + `coordinator_synthesis` job + periodic `hubspot_periodic_sync` cron + worker retry/concurrency policies + telemetry dashboard.
   - Day 3-4 — observation clustering + cross-deal pattern detection.
   - Day 5 — intelligence dashboard UI.
