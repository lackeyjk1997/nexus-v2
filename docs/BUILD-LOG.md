...
- **Phase 4 Day 2 first cron-firing window — empirical leak-fix verification.** Capture `pool-snapshot --label=phase4_day2_5min_post_deploy` 5 minutes after Phase 4 Day 2 deploy + `--label=phase4_day2_30min_post_deploy` 30 minutes after. Observe whether `candidate_leaks_60s_plus_excl_supavisor_recycle` stays at 0 (post-mitigation expectation) or grows over time (would indicate residual leak source not addressed by Mitigations 1+2). If pattern shifts, follow-up session targets remaining surfaces — but inventory already shows shared pool as the only remaining warm-container pool consumer.
