...
## Current state (as of 2026-04-27 — Phase 3 Day 4 Session B mid-session, deploy-then-PHASE-3-retry path)

- **Phase / Session in progress:** Phase 3 Day 4 Session B — pipeline wiring + verification + Vercel deploy closeout. Items 1–6 SHIPPED: handlers.ts collapsed step names per Meta-F (single "analyze" outer-step entry while the internal 3-way Promise.all stays intact); `IntelligenceCoordinator.receiveSignal` no-op fanout wired as step 5; step 6 `synthesize` runs a 2-way `Promise.all` over 06a-close-analysis-continuous + email-draft (post_pipeline); step 7 `persist_theory_email` appends `deal_theory_updated` + `email_drafted` events with `event_context` populated and source_refs scoped to `${transcriptId}:theory:${jobId}` + `${transcriptId}:email:${jobId}` per the §2.16 append-only discipline. Result POJO expanded: `stepsCompleted` 8 entries, `stepsDeferred` empty, new `coordinator/theory/email/email_full` summary surfaces, `claude` per-call telemetry expanded to 5 entries, timing fields added for the three new step boundaries. Mock harness updated to 5 fixtures (06a + email-draft inlined verbatim from `test-mock-claude.ts`) + 8-step assertion + per-event cumulative-count check after PHASE 2; live harness PHASE 1.5 fanout assertion extended 3→5 prompt_call_log rows + PHASE 3 worker-path also asserts 5 rows. `06a-close-analysis-continuous` reactive-bumped `1.1.0 → 1.2.0` after the first live PHASE 1 hit `stop_reason=max_tokens` at 1500 — bumped `max_tokens: 4000` per §2.13.1 reactive-bump policy. `DealIntelligence.getCurrentTheory` gained snake_case→camelCase transform so consumers see a single shape regardless of persistence format. Dev-script DIRECT_URL forced-swap removed in 4 scripts + DIRECT_URL fallback inverted in 3 scripts (now prefer pooler URL): the dev-Mac IPv6 route to Supabase direct host is broken (ping6 → "No route to host"), so option (a) DNS fix (`dns.setDefaultResultOrder("ipv6first")`) was applied as kickoff Decision 1 specified but reachability remained blocked at the routing layer; user-approved option (c) fallback per the parked-item resolution path: switch dev-script paths to pooler URL for this session's regression-check window. **Verification status:** mock harness 3/3 PHASES PASS; live PHASE 1 + 1.5 + 2 PASS at 8-step shape with 5 distinct prompt_call_log rows verified live (matching anchors, no errors); PHASE 3 worker-dispatched first attempt returned 622ms idle (localhost worker found no queued job — pg_cron prod likely claimed first via stale `e0ef9b2` Day 2 handler), then pool saturated at 200-client EMAXCONN and stayed saturated for 25+ min despite dev-server kill + sustained-drain Monitor; user-approved reorder: deploy first (Item 7) so prod runs Day 4 handler, then retry PHASE 3 once pool drains.
- **Next milestone:** Vercel auto-deploy on push to main → auth-gate smoke (curl prod /api/jobs/worker with bad bearer → expect 401) → wait for pool drain (deploy cycles old-prod-container connections; if drain stalls, fallback path is `cron.unschedule('nexus-worker')` for the retry window then re-schedule) → **single-attempt PHASE 3 retry**. Clean signal per oversight: pass = Session B closeout; fail = real worker-path bug, escalate without re-attempt.
...
- **Phase 3 Day 4 Session B:** **mid-session — deploy-then-PHASE-3-retry path active.**
...
- **PHASE 3 worker-dispatched — DEFERRED to post-deploy retry.** First attempt: `/api/jobs/enqueue` succeeded, Realtime subscribed, localhost worker triggered manually via curl, worker route returned 200 in 622ms — too fast for a Day 4 pipeline (~95s) — meaning the localhost worker found NO queued job to claim (pg_cron prod claimed first, racing the 10s schedule window). Test deadline 180s for Realtime UPDATE → no events received. The job (`37339b95-…`) was likely processed on prod's stale `e0ef9b2` Day-2 handler (which doesn't have Day 4 prompt files like 06a, email-draft, pipeline-extract-actions, pipeline-score-meddpicc); investigation queries blocked by the Supabase pooler hitting the 200-client EMAXCONN cap. Pool stayed saturated for **25+ minutes** despite dev-server kill + sustained-drain Monitor (typical drain is 2–5 min per the operational note — Day 3 PHASE 3 hit the same race + drain pattern but resolved briefly). User-approved reorder per kickoff broader escalation rule: deploy first (Item 7) so prod runs the same Day 4 handler, then retry PHASE 3 once pool drains. Pipeline-wiring already verified live by PHASE 1 + 1.5 (the 5-call fanout runs identically through worker dispatch — only the wrapping changes); the post-deploy retry is a single-attempt clean-signal gate per oversight discipline (fail = real worker-path bug, escalate; pass = Session B closeout).
...
- `pnpm --filter @nexus/db test:transcript-pipeline` — **PHASE 1 + 1.5 + 2 PASS live; PHASE 3 deferred to post-deploy retry.** 5-call fanout verified live with 5 prompt_call_log rows + matching anchors.
- Hex grep + stale shadcn class grep — 0 hits.
...
**Parked items closed.**
- Day 4 Session A "Session B scope" — most items CLOSED; PHASE 3 worker-dispatch verification deferred to post-deploy retry per oversight reorder.
- Day 2 Session B `outputFileTracingIncludes` parked — addressed by Item 7's auth-gate smoke (verifies prompts load on prod); fallback config in place if smoke surfaces missing-file errors.
- Day 4 Session A IPv6/DIRECT_URL parked — resolved via option (c) pooler-URL fallback (after option (a) DNS fix was applied per Decision 1 but didn't unblock at the network layer due to broken dev-Mac IPv6 routing).

**Parked items added.**
- **PHASE 3 worker-dispatch verification — deferred to post-deploy retry.** Single attempt; clean signal per oversight: fail post-deploy with current handler on both ends = real worker-path bug, escalate without re-attempt. Pass = Session B closeout.
- **Pool-saturation persistent-drain pattern.** Day 3 + Session A + Day 4 Session B all hit EMAXCONN; Day 3 + Session A drained briefly; Day 4 Session B drain stalled past 25 min. Root cause hypothesis: accumulated stale-Day-2-handler runs on prod under pg_cron's 10s schedule taking 30-40s each → 3-4 concurrent handlers per Vercel container × N containers under load. Post-deploy verification (this session's outcome) plus Phase 5+ may want a more aggressive shared-pool max-connection reduction or a pooler-saturation circuit breaker.
...
- **Phase 5 Day 1 06a `reasoning_trace` review trigger NOT fired in practice.** Day 4 Session B's live theory updates produced reasonable reasoning via per-section `triggered_by_quote` (the §2.13.1 calendared-default rationale). The calendared review still happens at Phase 5 Day 1 kickoff per §2.13.1; today's evidence is "leave-as-is is working." No early escalation needed.
- **Dormant-trigger activation discipline for email-draft** — unchanged from Session A.

**Cost.** Live Claude: 5 calls × 2 PHASE-1+2 runs in sub-step 2 ≈ 10 calls × ~$0.07/call ≈ ~$0.70. Plus Item 7 Vercel deploy ($0 — Hobby tier). Plus PHASE 3 retry (5 more calls if it runs ≈ $0.35). Projected total ≈ $1.05 (within kickoff budget of $1.40-$1.70). Voyage $0 (preprocessor short-circuits idempotently). HubSpot $0 (PATCH cost negligible at 9 properties × 2 runs = 18 property-writes; PATCH counts as 1 call against 100/day rate-limit headroom).
