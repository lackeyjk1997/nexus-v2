...
## Current state (as of 2026-04-27 — Phase 4 Day 1 Session A.5 complete · Session B ahead)
...
- **Next milestone:** **Phase 4 Day 1 Session B — admission + scoring + verification staircase.** Inherits the locked DSL + evaluator + DealState as foundation inputs PLUS the now-correct `event_context` value-side (admission engine reads segmentation gating; coordinator pattern detection slices by accurate-at-event segmentation). Lands the surfaces registry literal port (rebuild plan lines 470-494), admission engine via `SurfaceAdmission.admit(...)`, scoring prompt body + tool schema, `DealIntelligence.getApplicablePatterns/Experiments/Flags` methods composing the evaluator + table reads. See **`## Forward map`** section for downstream sequence.
...
2. **Phase 4 — Intelligence layer** (~5 days; Day 1 splits A/A.5/B per execution decisions).
   ...
   - **Day 1 Session B** (NEXT) — surfaces registry + admission engine + scoring prompt + `DealIntelligence.getApplicable*` methods + verification staircase. Inherits accurate `event_context` from Session A.5.
