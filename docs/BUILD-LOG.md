...
- **Latest commit on `main` (nexus-v2):** `e5f8854 feat(phase-2-day-4-session-a): stakeholder management + 2 contact stub promotions + 2 deal-contact association methods`.
...
- **Vercel production:** Still on `e0ef9b2` until Jeff green-lights Session A deploy. Session A has no new routes; `/pipeline/[dealId]` grew 4.1 kB → 5.98 kB first-load JS for the stakeholder card + 4 server actions. 13 routes unchanged.
