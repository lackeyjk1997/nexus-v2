...
## Current state (as of 2026-04-28 — Phase 4 Day 3 closed · Phase 4 Day 4 ahead)
...
- **Phase 4 Day 3 complete (Path A conclusive).** Observation clustering substrate + category-candidate admission surface shipped: 10-cluster-observations prompt (v1.0.0, classification temp 0.2 max_tokens 1500, reasoning_trace first per §2.13.1) + cluster-observation tool + observation_cluster job handler replacing `notYet` stub + `nexus-observation-cluster` cron entry every 30 min via configure-cron extension. Migration 0007 lands the candidate-shape extension to observation_clusters (drop NOT NULL on signal_type+severity; add cluster_key UNIQUE + 9 candidate columns + status index). Surfaces registry extended to 5 surfaces (4 → 5; new `category_candidates` portfolio surface with `{minMemberCount: 3, minConfidence: "medium"}` per §1.16 LOCKED + §1.18). SurfaceAdmission gains `admitCategoryCandidates` portfolio branch. AdmissionCandidate union extended with `category_candidate` kind. All gates green: 4/4 typecheck + 13 routes clean build + enum:audit + 78/78 baseline cases unchanged + **8/8 new** (test:cluster-observation 5/5 + test:category-candidates-admission 3/3) + **test:surfaces-registry 11/11** (was 8/8; +3 cases). Live exercise `test:cluster-observations-medvista` PASS via Path B synthetic-injection (prod has 0 observations per Decision 8): 3 seeded uncategorized observations → 3 sequential Claude calls produced same `integration_timeline_anxiety` signature with confidence `high` (Decision 4 determinism contract verified empirically) → cluster row written + 3 back-link UPDATEs → cleanup unconditional (3 obs + 1 cluster deleted post-exercise; prod state restored). Live Claude budget: ~$0.06 (within $1.50 cap per Decision 5). $0 Voyage / $0 HubSpot. Membership tracking diverged from kickoff Decision 6 option (b) — used existing observations.cluster_id FK (option (c)) per Guardrail 13 single-write-path, captured in Reasoning stub. Pool-snapshot 5-min + 30-min post-deploy gate pending; handler runs in-process locally so the live exercise was complete pre-deploy. **Pre-Phase-4-Day-2 leak fix continues to hold** — preflight pool-snapshot showed only the 2 known unrelated entries; no new sustained-load surface added beyond the 30-min cron entry.
- **Next milestone:** **Phase 4 Day 4** — scope to be adjudicated at Day 3 closeout per kickoff Decision 1. Candidates: (a) coordinator prompt context enrichment (carried-forward parked item from Phase 4 Day 2 Session A); (b) cross-deal pattern detection beyond the coordinator MVP (temporal patterns, stage-velocity drift); (c) fold into Day 5 dashboard prep (read substrate hardening for Day 5's surfaces). See **`## Forward map`** section for downstream sequence.
...
- **Phase 4 Day 2 Session B:** **complete (Path A conclusive).** Phase 4 Day 2 fully closed.
- **Phase 4 Day 3:** **complete (Path A conclusive).** Phase 4 Day 4 ahead — scope adjudicated at closeout.
- **Latest commit on `main` (nexus-v2):** `c1a97d3 feat(phase-4-day-3): observation clustering + category-candidate admission surface`. Builds on `2937155 docs: seed-data realism (BUILD-LOG) + customer company-knowledge layer (PRODUCTIZATION)` → `a85bc97 docs(build-log): operational note — overnight verification windows for novel load patterns` → `82ed2fc docs(build-log): Phase 4 Day 2 Session B closeout + interruption-recovery snapshot` → `cf2c9c2 feat(phase-4-day-2-session-b): hubspot_periodic_sync + worker retry/loop/sweep + wrapper protocol-retry`.
...
## Forward map (as of 2026-04-28 — Phase 4 Day 3 closed; Day 4 ahead)
...
3. **Phase 4 — Intelligence layer** (~5 days; Day 1 splits A/A.5/B per execution decisions).
   ...
   - **Day 3** ✓ COMPLETE (*pending commit*) — 10-cluster-observations prompt + cluster-observation tool + observation_cluster job handler (replaces `notYet`) + migration 0007 (observation_clusters candidate-shape extension: drop NOT NULL on signal_type+severity; add cluster_key UNIQUE + 9 candidate columns + status index) + surfaces registry extended to 5 surfaces (new `category_candidates` portfolio surface) + SurfaceAdmission.admitCategoryCandidates branch + new AdmissionCandidate kind + `nexus-observation-cluster` cron entry every 30 min via configure-cron. 8/8 new test cases (5+3); test:surfaces-registry 8/8 → 11/11. Live exercise PASS via Path B synthetic-injection (prod has 0 observations per Decision 8): 3 Claude calls produced same `integration_timeline_anxiety` signature with confidence `high` — Decision 4 determinism verified empirically. Cluster written + 3 back-link UPDATEs + unconditional cleanup. ~$0.06 live Claude (within $1.50 cap). Pool-snapshot 5-min + 30-min post-deploy gate pending.
   - **Day 4** (NEXT) — scope to be adjudicated at Day 3 closeout per kickoff Decision 1. Candidates: (a) coordinator prompt context enrichment (carried-forward parked item from Phase 4 Day 2 Session A — `getAtRiskComparableDeals`, `getActiveManagerDirectives`, `getSystemIntelligence` helpers; affectedDealsBlock CrmAdapter enrichment); (b) cross-deal pattern detection beyond the coordinator MVP (temporal patterns, stage-velocity drift); (c) fold into Day 5 dashboard prep (read substrate hardening for Day 5's surfaces).
   - Day 5 — intelligence dashboard UI (consumes admitted set + scores from `SurfaceAdmission.admit` for both `intelligence_dashboard_patterns` AND `category_candidates`) AND telemetry dashboard `/admin/claude-telemetry` (re-deferred from Session B).
...
**Phase 4 Day 3: ALL CLOSED.**
- ~~`observation_cluster` job handler wiring~~ ✓ Day 3 (`observationCluster` JobHandler + 10-cluster-observations prompt + cluster-observation tool + migration 0007)
- ~~`category_candidates` admission surface~~ ✓ Day 3 (5th surface in registry + admitCategoryCandidates branch + AdmissionCandidate union extended + 11/11 test:surfaces-registry)
- ~~`nexus-observation-cluster` cron entry~~ ✓ Day 3 (`*/30 * * * *` via configure-cron extension)

**Phase 4 Day 4 (next session — scope adjudicated at Day 3 closeout):**
- coordinator_synthesis prompt context enrichment — wire `getAtRiskComparableDeals`, `getActiveManagerDirectives`, `getSystemIntelligence` helpers; affectedDealsBlock CrmAdapter enrichment for stage/AE/stakeholders. Today the prompt receives placeholder strings for these blocks per its documented empty-block conventions. Carried forward from Phase 4 Day 2 Session A.
- OR cross-deal pattern detection beyond the coordinator MVP (temporal patterns, stage-velocity drift)
- OR fold into Day 5 dashboard prep (read substrate hardening for Day 5's surfaces)
...
**Phase 5+ (Marcus's promotion UI for category candidates — added Phase 4 Day 3):**
- Reads `observation_clusters WHERE status='candidate'` + member observations via `observations.cluster_id`. UI shows cluster + signature + signature_basis + Claude's confidence + member-observation snippets; Marcus picks promote (UPDATE status='promoted' + assign signal_type from canonical taxonomy + set severity) or reject (UPDATE status='rejected'). Out of v2 demo scope per kickoff (and §1.16's "human curation reserved for one case" lock).
- When Marcus's promotion adds a new canonical signal type, the schema's `signal_taxonomy` pgEnum needs a value added via migration. Not v2 demo scope — initial 9 types are the seed; clusters that don't fit may be repeatedly proposed as candidates until Marcus either promotes them (extending the taxonomy) or systematically rejects them.
...
| Seed deal count for demo-meaningful coordinator_patterns | Phase 4 Day 5 prerequisite (decide at Day 2 Session B / Day 3 closeout latest) | Pattern detection requires `minDealsAffected >= 2`; MedVista alone produces 0 patterns + empty dashboard. Decide seed deal count + verticals + overlapping signal types + creator before Day 5 kickoff. Day 3 closeout: deferred adjudication; Phase 4 Day 4 scope decision impacts whether seeding lands earlier. |
| `10-cluster-observations` max_tokens reactive bump | Phase 4 Day 5 / Phase 5+ live cron firings | 1500 starting budget; reactive bump if `stop_reason=max_tokens` fires from cron-triggered runs against real corpus. |
| Observation seed count for demo-meaningful category_candidates | Phase 4 Day 5 prerequisite | category_candidates surface requires 3+ uncategorized observations sharing a signature. Today's prod has 0 observations; live exercise used synthetic injection. Demo-grade requires either (a) seeded uncategorized observations across multiple deals OR (b) a captured-via-rep observation flow seeded with realistic input distribution. Decision interacts with seed-data realism note (section above) — solve together at the seeding conversation kickoff. |
| Decision 4 signature determinism at scale | Phase 5+ / Stage 3 corpus volume | Live exercise empirically verified determinism at 3 same-shape observations (3/3 produced same signature within temperature 0.2). At Stage 3 corpus volume (10K+ observations), revisit whether prompt-generated signatures hold determinism well enough — or whether to migrate to embedding-based clustering (Decision 5 deferred Voyage usage to Phase 5+). |
| `observations.cluster_id` membership tracking pattern | Phase 5+ Marcus promotion UI | Day 3 chose option (c) — existing FK — over Decision 6's option (b) new join table. Re-evaluate if Phase 5+ promotion UI needs richer membership metadata (e.g., per-membership confidence, per-membership signature variance). Today the cluster carries one signature; promotion UI may want per-member views. |
| Wrapper `claude_call_log_write_failed` second observable cause | Stage 4 SOC 2 audit-trail-durability productization | Day 3 added a second cause (UUID format mismatch on jobId anchor) to Phase 4 Day 1 Session B's first cause (pool saturation). Both manifest the same silent-skip behavior; productization fix shape (durable WAL + retry + backpressure per PRODUCTIZATION-NOTES.md SOC 2 section) addresses both uniformly. |
...
### Phase 4 Day 3 — 2026-04-28 · `c1a97d3`

**Observation clustering + category-candidate admission surface shipped.** Both Item slots from Day 3's expected scope landed: Item 1 (10-cluster-observations prompt + cluster-observation tool + observation_cluster job handler replacing the `notYet` stub + cron entry) and Item 2 (surfaces registry extended with new `category_candidates` portfolio surface + admission engine portfolio branch reading `observation_clusters`). Migration 0007 lands the candidate-shape extension to the existing `observation_clusters` table per Decision 6 (drop NOT NULL on signal_type + severity; add cluster_key UNIQUE + normalized_signature + candidate_category + confidence + signature_basis + vertical + status + member_count + last_synthesized_at + applicability JSONB). The kickoff anticipated this migration. Internal verification staircase green; live exercise PASS via Path B synthetic-injection per Decision 8 (prod has 0 observations). Live Claude budget consumed: ~$0.06 across 3 sequential calls (within $1.50 cap per Decision 5).

**Item 1 — `observation_cluster` job handler (replaces `notYet` stub).** Reads uncategorized observations (`signal_type IS NULL AND cluster_id IS NULL`, LIMIT 100 defense), generates a normalized signature for each via 10-cluster-observations + the new `cluster_observation` tool. Skips low-confidence rows per §1.18. Groups by `vertical|signature` (cross-vertical observations share a group only when verticals match). For each group with `member_count >= minObservationsPerCluster` (default 3 per §1.16 LOCKED + Decision 3): writes an `observation_clusters` row idempotent on `cluster_key = sha256(signature + vertical || "all").slice(0,32)` + UPDATEs each member's `observations.cluster_id` so subsequent runs skip. Membership tracking diverged from Decision 6 option (b) new-table proposal — the existing `observations.cluster_id` FK gives us option (c) without doubled state per Guardrail 13. New `cluster-observation.ts` tool definition mirrors the prompt's tool-use schema (reasoning_trace + normalized_signature + candidate_category + confidence + signature_basis) per §2.13.1 Principle 6. Confidence escalation in groups: any "high" member promotes the group's confidence to "high"; otherwise "medium" (low rows already skipped).

**Item 2 — Surfaces registry extension + admission engine branch.** `category_candidates` is the 5th surface (4 existing + 1 new). Portfolio kind. Threshold shape: `{minMemberCount: 3, minConfidence: "medium"}` per Decision 7. New `CategoryCandidatesSurface` discriminated TS member; `SurfaceConfig` union expanded; `SURFACE_IDS` array now 5 entries. `SurfaceAdmission.admit` portfolio path now branches on `surface.id === "category_candidates"` to call the new `admitCategoryCandidates` method (reads `observation_clusters` filtered by status='candidate' AND member_count >= threshold, applies in-memory confidence-floor filter per §1.18 belt-and-suspenders, sequential per-candidate Claude scoring per existing 09-score-insight pattern, sorts by score desc, truncates to maxItems). New `category_candidate` AdmissionCandidate kind; `CategoryCandidateCluster` interface; `serializeCandidate` extended; `truncateToMaxItems`'s call_prep_brief bucket gains a category_candidate slot for type-completeness (always empty for that surface; portfolio-only).

**Migration 0007 applied live.** Pre-execution preflight verified `observation_clusters.row_count=0` (NOT NULL constraints + UNIQUE on cluster_key safe without backfill). Migration applied via `apply:migration-0007` script following the migration-0006 idempotence-check precedent. 14 statement blocks; all post-migration columns + UNIQUE constraint + status index verified. observation_clusters now carries 20 columns (was 11); UNIQUE constraint `observation_clusters_cluster_key_unique`; new `observation_clusters_status_idx` index.

**Cron extended.** `configure-cron` now schedules `nexus-observation-cluster` at `*/30 * * * *` alongside the existing 10-second `nexus-worker` and 15-minute `nexus-hubspot-sync`. Cadence per Decision 2 — observations accumulate slowly (no real-time freshness needed); 30-min strikes the balance. Cron body: `INSERT INTO public.jobs (type, status) SELECT 'observation_cluster', 'queued' WHERE NOT EXISTS (SELECT 1 FROM jobs WHERE type='observation_cluster' AND status IN ('queued','running'))`. Same NOT EXISTS dedup guard as nexus-hubspot-sync.

**Telemetry per Decision 10.** Six new stderr JSON event shapes:
  - `observation_cluster_started` (job_id, total_observations_read, vertical_filter, min_observations_per_cluster, observation_limit)
  - `observation_signature_generated` (per observation: id + signature + category + confidence)
  - `observation_cluster_low_confidence_skipped` (per low-confidence row)
  - `observation_cluster_below_threshold` (per group below threshold: signature, member_count, threshold)
  - `observation_cluster_emitted` (per cluster: id, key, signature, category, confidence, member_count, vertical)
  - `observation_cluster_completed` (clusters_emitted + signatures_generated + low_confidence_skipped + below_threshold + duration_ms)

**Internal verification staircase (all gates green).**
- `pnpm typecheck` — 4/4 workspaces PASS.
- `pnpm build` — 13 routes clean, zero `Attempted import error | Module not found | Type error | Failed to compile`.
- `pnpm --filter @nexus/db enum:audit` — PASS.
- 13 baseline gates × all cases unchanged (12+8+8+9+8+8+5+3+4+5+4+4 = 78/78 PASS): applicability-dsl 12/12, deal-state 8/8, build-event-context 8/8, applicable-pattern-experiment-flag 9/9, surface-admission 8/8, coordinator-receive-signal 8/8, coordinator-active-patterns 5/5, coordinator-synthesis 3/3 PHASES, hubspot-periodic-sync 4/4, worker-retry-policy 5/5, worker-loop-until-empty 4/4, wrapper-protocol-retry 4/4.
- **`pnpm --filter @nexus/shared test:cluster-observation` — 5/5 PASS** NEW (empty observations + silence-as-feature; 1 obs sub-threshold; 3 obs same signature → 1 cluster + back-link UPDATE; 3 obs different signatures → 0 clusters; mixed high/low confidence → low filtered + group confidence escalated to high).
- **`pnpm --filter @nexus/shared test:category-candidates-admission` — 3/3 PASS** NEW (no clusters → empty admitted + silence; 1 qualifying cluster → 1 admitted with score; below member-count + below confidence → both filtered).
- **`pnpm --filter @nexus/shared test:surfaces-registry` — 11/11 PASS** UPDATED (was 8/8; +3 cases for category_candidates literal shape + getSurface narrowing + threshold anchor to §1.16/§1.18).
- `audit-event-context-fields` — 0 vertical-null rows (Session A.5 closure verification holds).
- Hex grep on .tsx files — 0 hits. Stale shadcn class grep — 0 hits.

Total: 86/86 cases across 14 test gates (typecheck + 12 baseline + 2 new + 1 updated).

**Configure-cron applied to prod.** `pnpm --filter @nexus/db configure-cron https://nexus-v2-five.vercel.app` succeeded; three cron entries confirmed: `nexus-worker` (every 10s) + `nexus-hubspot-sync` (every 15 min) + `nexus-observation-cluster` (every 30 min, `*/30 * * * *`).

**Live exercise: `test:cluster-observations-medvista` — Path B SYNTHETIC-INJECTION PASS.** Per Decision 8 (prod has 0 observations; Path A silence-path uninformative). Seeded 3 uncategorized observations with similar underlying shape (all describe pre-purchase anxiety about EMR/Epic integration timeline). Handler invoked against prod Supabase + live Claude wrapper. Result: `clustersEmitted=1, observationsRead=3, signaturesGenerated=3, lowConfidenceSkipped=0, belowThreshold=0, durationMs=29779`. All 3 Claude calls produced the SAME signature `integration_timeline_anxiety` with confidence `high` — Decision 4's determinism contract verified empirically against real Claude (3/3 same-shape observations → same signature within temperature 0.2 bounds). Cluster row written with cluster_key, member_count=3, vertical=healthcare, status=candidate. All 3 observations.cluster_id back-linked. Telemetry trail FULL: `observation_cluster_started` × 1 → `observation_signature_generated` × 3 → `observation_cluster_emitted` × 1 → `observation_cluster_completed{clusters_emitted=1}` × 1. Cleanup unconditional (try/finally) — 3 observations + 1 cluster deleted post-exercise; prod state restored to 0 observations / 0 clusters as preflight.

The live exercise's ONE observation: the 3 Claude calls each fired `claude_call_log_write_failed` with `invalid input syntax for type uuid: "phase4-day3-live-XXX"` — the test script passed a non-UUID jobId anchor, the wrapper's best-effort contract caught it, the Claude call itself succeeded, prompt_call_log writes silently skipped. Cron-fired production runs use `jobs.id` (real UUID) so prompt_call_log writes will land normally. Script updated post-exercise to use `crypto.randomUUID()` for re-runs. The wrapper's best-effort contract working as designed (PRODUCTIZATION-NOTES.md "Audit-trail durability under pool saturation — Stage 4 SOC 2 surface" — same productization arc, different cause: format mismatch instead of pool saturation, both manifest the same silent-skip behavior).

**Post-deploy pool-snapshot gate (Decision 9 minutes-scale, NOT overnight).**
- `pool-snapshot --label=phase4_day3_5min_post_deploy` (5 min after push at 2026-04-28T15:38Z; captured at 2026-04-28T15:44:25Z): `total_connections=14`, `candidate_leaks_60s_plus_excl_supavisor_recycle` shows the 2 known unrelated long-held entries (postgrest LISTEN PID 3610 at 525182s ≈ 6 days; admin "show archive_mode" PID 46958 at 45565s ≈ 12.6 hours) PLUS one third entry: PID 608431, postgrest IDLE COMMIT, 1592s ≈ 26.5 min. The third entry is in the SAME class as the existing PID 3610 (both `application_name="postgrest"`, both Supabase-managed REST layer backends, neither produced by our app's postgres.js shared pool). Total_connections went from baseline 13 → 14 (one additional postgrest backend held idle by the Supabase REST layer). Not an app-side leak — the leak fix's structural class (`createDb` per-invocation × `idle_timeout: 0`) is closed; new entry is a different class (PostgREST connection-pool warmup or Supabase dashboard activity). EMAXCONN remains the ground-truth saturation signal and is not present.
- 25-minute QUIET PERIOD between snapshots — used productively for BUILD-LOG entry composition + commit-hash backfill per Decision 9 discipline.
- `pool-snapshot --label=phase4_day3_30min_post_deploy` (captured at 2026-04-28T16:24:02Z, ~25 min after 5-min snapshot, ~40 min after push): `total_connections=13` (DOWN from 14 at the 5-min mark — back to preflight baseline), `candidate_leaks_60s_plus_excl_supavisor_recycle` shows ONLY the 2 known unrelated entries (postgrest LISTEN PID 3610 at 527559s; admin "show archive_mode" PID 46958 at 47942s). The PID 608431 postgrest IDLE COMMIT entry from the 5-min snapshot is GONE — confirms it was transient (Supabase REST layer connection-pool warmup that closed within the 25-min window) NOT a sustained leak.

**PASS at both windows.** Zero new app-side leaks accumulated. The 5-min snapshot's third entry was empirically validated as transient at the 30-min snapshot — the leak fix continues to hold under Day 3's added load (1× cron entry every 30 min + handler runtime). The four-of-four EMAXCONN pattern remains structurally CLOSED across Phase 4 Day 1 + Day 2 Sessions A + B + Day 3.

**Reasoning stub.** Non-MVP choices with justification type per the CLAUDE.md reasoning-gate.

- **Decision 6 divergence: membership tracking uses existing observations.cluster_id FK rather than creating a new observation_cluster_memberships join table.** Justification 1 (Guardrail 13 — single write-path on observations + observation_clusters). The kickoff Decision 6 considered options (a) uuid[] (rejected per §2.2) and (b) new join table; missed option (c) which is the existing schema's shape. observations.cluster_id is already defined with ON DELETE SET NULL + a clusterIdx index. Adding a new join table would create double state. The handler updates cluster_id during the back-link UPDATE step; subsequent runs skip already-clustered rows via the WHERE cluster_id IS NULL filter. Captured here as a Type 1 Reasoning entry rather than escalating since this is "small schema decision within an established framework" per CLAUDE.md.

- **Migration 0007 makes signal_type + severity nullable to support candidate-state rows.** Justification 2 (§2.13.1 nullable invariant precedent). The existing observation_clusters schema assumed clusters were already-promoted (signal_type from canonical taxonomy + severity set by Marcus). v2 demo's flow is the inverse — clusters START as candidates without a canonical signal_type, transition to promoted state when Marcus picks one. Making both nullable preserves the post-promotion shape (Marcus-set values write through normally) while supporting the candidate-state shape. The §2.13.1 `observations.signal_type NULL` precedent applies the same pattern at the row level; this extension applies it at the cluster level.

- **`cluster_key = sha256(signature + vertical || "all").slice(0, 32)` for idempotency.** Justification 1 (Guardrail 13 — single write-path on observation_clusters) + Justification 2 (§2.16 append-only-events spirit; same group = same row). 32-char prefix gives 128 bits of collision resistance, sufficient for v2's expected cluster volume. Sorting verticals via the `|| "all"` convention ensures cross-vertical clusters don't collide with vertical-scoped clusters. Mirrors the coordinator_synthesis `pattern_key` shape at the cluster level.

- **Per-observation Claude calls are SEQUENTIAL, not Promise.all.** Justification 1 (§2.6 simplicity) + Justification 4 (45-connection working ceiling per Pre-Phase-4-Day-2 finding). The handler is bounded by `observationLimit=100` worst case; Promise.all over 100 Claude calls would multiply pool footprint × Claude rate-limit pressure. Sequential keeps both surfaces predictable. The kickoff specifically called out per-cluster fanout as a Promise.all candidate but Decision 4's per-observation prompt shape means Claude calls happen BEFORE grouping, not per-group.

- **Confidence escalation: group confidence = max(member confidences).** Justification 4 (next-session need — surfaces UI ordering). When a group has both medium + high confidence members, the cluster's surfaced confidence should reflect the strongest signal — Marcus reads confidence as a quick filter. If 2 members are high and 1 is medium, the cluster is high-confidence overall. Low rows are filtered out at the per-observation step so they never participate. Captured as a Reasoning entry because the kickoff didn't specify the escalation rule explicitly.

- **`title` derived from candidate_category, not separately requested in tool output.** Justification 1 (§2.13.1 max_tokens budget discipline). The kickoff Decision 4 listed 5 tool output fields; adding a 6th (`title`) would inflate token budget without value — candidate_category is already title-case-able. Handler uses candidate_category as title directly. The DB row carries both for forward-compat (Marcus's promotion UI may want to edit title separately from the canonical candidate_category lineage).

- **In-memory confidence-floor filter is belt-and-suspenders against handler regressions.** Justification 1 (Guardrail 51 — silence-as-feature low-confidence rows). The handler skips low-confidence rows at signature-generation time so low-confidence cluster rows shouldn't reach the surface. But the threshold filter at admission applies the floor again — defense-in-depth against future handler changes that might admit low-confidence to satisfy a new requirement. Cost: trivial in-memory filter; benefit: surface-level §1.18 contract holds independently of handler's filter discipline.

- **Test script uses non-UUID jobId initially; then updated to crypto.randomUUID() post-discovery.** Justification 4 (imminent next-session need — re-runs of the live exercise). The first live exercise revealed the wrapper's best-effort contract silently skips prompt_call_log writes on non-UUID anchors. Cron-fired production runs use real `jobs.id` UUIDs so the issue doesn't surface there. Test scripts are the only path that uses synthetic jobIds. Updated the script to use randomUUID() so re-runs surface prompt_call_log audit rows. The discovery is informative for the productization-arc's SOC 2 audit-trail-durability decision — adds a second-cause example to the existing pool-saturation-cause example in PRODUCTIZATION-NOTES.md.

- **`category_candidate` reuses 09-score-insight without rewrite — verified at pre-execution.** Justification 1 (Guardrail 13 — single scoring prompt). 09-score-insight is general-purpose; the prompt body interpolates `${candidateInsightBlock}` which I extended via serializeCandidate to include the cluster's signature + member_count + confidence + signature_basis. No prompt rewrite or version bump needed. Phase 5+ admin-tuning UI may want a category-specific scoring rubric, in which case it'd land as a 1.0.0 → 1.1.0 bump per the §2.13.1 versioning discipline. Today's reuse pattern proven against the cluster-observation live exercise's signature determinism — same shape produces same signature, which is the substrate this scoring serves.

- **`category_candidates` uses portfolio kind despite being downstream of per-deal observations.** Justification 3 (PRODUCTIZATION-NOTES.md "Corpus Intelligence — the second product" — taxonomy evolution). A cluster spans many observations from many deals; the cluster IS the portfolio-scope artifact. Applicability gating doesn't apply — there's no single deal context. The admission engine routes through admitCategoryCandidates which mirrors admitPortfolio's coordinator_patterns reader at a different table. Stage 3+ corpus-narrative analysis reads observation_clusters across time as the taxonomy-evolution substrate; today's WRITE shape locks the schema for that future reader.

10 entries. No UNCERTAIN.

**Parked items closed.**

- **`observation_cluster` handler wiring** — landed via `observationCluster` JobHandler replacing the `notYet` stub. `10-cluster-observations` prompt has `reasoning_trace` first per §2.13.1 Principle 6; max_tokens=1500 held — no reactive bump fired during the live exercise (3/3 Claude calls returned tool_use cleanly).
- **observation_clusters candidate-shape extension** — landed via migration 0007 (drop NOT NULL on signal_type + severity; add cluster_key UNIQUE + 9 candidate-shape columns + status index).
- **`category_candidates` surfaces registry surface** — landed; SURFACES.category_candidates with portfolio kind + threshold shape.
- **SurfaceAdmission category_candidates branch** — landed via admitCategoryCandidates method + extended AdmissionCandidate union + extended serializeCandidate + extended applyPreScoringThresholds.
- **`nexus-observation-cluster` cron entry** — landed via configure-cron extension (`*/30 * * * *`).

**Parked items added.**

- **Phase 4 Day 4 — to be adjudicated at Day 3 closeout per kickoff Decision 1.** Two candidate scopes carrying forward: (a) coordinator prompt context enrichment (`getAtRiskComparableDeals`, `getActiveManagerDirectives`, `getSystemIntelligence` helpers; affectedDealsBlock CrmAdapter enrichment for stage/AE/stakeholders) — Phase 4 Day 2 Session A's parked item; (b) cross-deal pattern detection beyond the coordinator MVP (e.g., temporal pattern detection, stage-velocity drift). Alternative path: fold Day 4 into Day 5 dashboard prep (read substrate hardening for Day 5's surfaces). Adjudication deferred to Day 4 kickoff.
- **Phase 5+ — Marcus's promotion UI for category candidates.** Reads `observation_clusters` rows where `status='candidate'` + member observations via `observations.cluster_id`. UI shows cluster + signature + signature_basis + Claude's confidence + member-observation snippets; Marcus chooses promote (assign canonical signal_type from the 9-type taxonomy + severity) or reject (status='rejected'). Promotion path: UPDATE observation_clusters SET signal_type=$picked, severity=$picked, status='promoted'. Rejection: UPDATE status='rejected'. Out of v2 demo scope per kickoff (and §1.16's "human curation reserved for one case" lock).
- **Phase 5+ — observation_classifier signal_type enum extension when promoted clusters exceed the 9-type taxonomy.** When Marcus's promotion adds a new canonical signal type, the schema's `signal_taxonomy` pgEnum needs a value added via migration. Not v2 demo scope — initial 9 types are the seed; clusters that don't fit may be repeatedly proposed as candidates until Marcus either promotes them (extending the taxonomy) or systematically rejects them.
- **Phase 4 Day 5 — `category_candidates` UI dashboard surface.** Reads admitted set from SurfaceAdmission.admit({surfaceId: "category_candidates", userId}). Renders ranked clusters with signature + signature_basis + member_count + confidence + score + scoreExplanation. Empty state per §1.18 — "No new category candidates have crossed threshold." UI work for Phase 4 Day 5 alongside the intelligence dashboard.
- **Stage 3 productization — corpus narrative analysis reads observation_clusters across time.** observation_clusters carries normalized_signature + last_synthesized_at + member_count over time. Stage 3+ corpus-intelligence dashboard reads this as a taxonomy-evolution surface ("new categories emerging by vertical, by quarter"). v2 demo's silence-as-feature default produces 0 clusters typically; productization volume produces the meaningful shape.
- **Stage 4 SOC 2 audit-trail-durability — wrapper best-effort contract has a second observable cause now.** Phase 4 Day 1 Session B observed `claude_call_log_write_failed` mid-EMAXCONN (pool saturation cause). Phase 4 Day 3 observes the same event mid-test-script-run (UUID format mismatch cause). Both manifest silent-skip on prompt_call_log writes; user-facing Claude call succeeds normally. The productization-arc fix (durable write-ahead log + retry + backpressure per the SOC 2 productization note) addresses both causes uniformly.

**Cost.** $0 Voyage (Day 3 doesn't use embeddings per Decision 5). **Live Claude:** ~$0.06 (3 sequential calls × ~$0.02 each; well under Decision 5's $1.50 cap). **Prod Supabase:** ~80-100 reads across pool-snapshot baseline + 3 cron schedule SELECTs + live-exercise pre/post state queries + audit-event-context-fields + handler observation read. Writes: 1 cluster INSERT, 3 observation cluster_id UPDATEs, 3 observation INSERTs (seeded), 4 deletes (cleanup: 3 obs + 1 cluster), 14 ALTER/INDEX statements via migration 0007, 3 cron schedule SELECT/INSERT pairs (existing nexus-worker + nexus-hubspot-sync unscheduled+rescheduled; new nexus-observation-cluster scheduled). $0 HubSpot.
...
## Seeding & demo data design considerations

Pre-kickoff thinking that informs upcoming session design but isn't itself implementation scope. Surfaces here so the seeding conversation kickoff (urgent before Phase 4 Day 5) sees the full thinking. Items here graduate to dedicated kickoffs or Forward-map parked items as they become actionable.

### Seed data realism — input distribution discipline (urgent before Phase 4 Day 5; full scope at seeding conversation kickoff)

Seeded transcripts and emails are not just demo content — they are the input substrate Nexus's prompts are exercised against during build verification. Clean seeded data risks training prompts against an unrealistic input distribution that breaks unpredictably on real customer data. Three specific failure-mode categories to design seed data against:

1. **Transcription artifacts.** Real transcription engines mangle acronyms, brand names, and technical jargon ("iCIMS" → "IMS" or "I sims"; "MEDDPICC" → "med pick"). Seeded transcripts should include these errors deliberately, not be artificially cleaned. Prompts need to handle uncertain entity recognition gracefully — inferring from context or surfacing uncertainty rather than confabulating specifics.
2. **Conversational artifacts.** Speaker overlap, [crosstalk] markers, half-finished sentences, interruptions, side conversations, filler words. Real transcripts are conversationally messy in ways generated demo data isn't. Seeded data should reflect this.
3. **Multi-channel quality variance.** Email is cleaner than transcription (read and edited before sending). Transcripts are decoded live with no editorial pass. Signal-extraction logic across channels should weight accordingly — high-confidence facts from emails, lower-confidence from transcripts unless corroborated. Seeded data spanning both channels should reflect this asymmetry.

**Domain specificity recommendation.** Use a real, identifiable company (e.g., Paradox in conversational AI for talent acquisition) as the seed-data subject rather than generic placeholder companies. Three reasons: (a) gives prompts genuine domain context to reason against; (b) lets the operator (Jeff) evaluate output sensibility against known ground truth; (c) prospect demos against credible-feeling real-company data are more persuasive than generic placeholder data. The seeded company should be one Jeff knows well enough to detect when the system is producing nonsense vs. valid analysis.

**Pre-Phase-4-Day-5 scope.** Decide seed company + verticals + minimum deal count for `coordinator_patterns` demo-meaningfulness (≥2 deals with overlapping signals). Generate seed transcripts that include realistic transcription artifacts (acronym mangling, [crosstalk], half-finished sentences). Generate seed emails as cleaner-than-transcripts equivalents. Verify prompts handle the input-distribution variance gracefully — score-insight grounding-discipline finding (Phase 4 Day 1 Session B) is one early warning of what happens when prompts encounter input gaps; seed-data realism is the upstream defense.

**Phase 5+ broader corpus implications.** As `coordinator_synthesis` scales beyond hand-curated seed data toward broader corpus content, the input-distribution discipline becomes load-bearing for productization. The seeding conversation should explicitly name what realistic-data exercises the system has been verified against vs. which input shapes remain unverified.

**Cross-reference.** The v2-demo-lightweight company-knowledge layer (`docs/PRODUCTIZATION-NOTES.md`, "Customer company-knowledge layer (Stage 2)") is the downstream defense — hand-curated company context fed into prompts as ambient grounding. Seed data realism + company-knowledge ambient context together address the input-grounding gap from both directions.
