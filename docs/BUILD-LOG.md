...
## Current state (as of 2026-04-28 — Phase 4 Day 1 Session B complete · Day 2 ahead)
...
- **Next milestone:** **Pre-Phase-4-Day-2 — Ops/diagnostic: recurring pool-saturation root cause** (own session, oversight-sequenced ahead of Phase 4 Day 2 per Session B closeout). Four-of-four sessions hit `EMAXCONN`. Decision 9 promoted from "parked item to investigate before Phase 4 Day 2" to "dedicated own session FIRST" so Phase 4 Day 2's net new baseline load (`coordinator_synthesis` + 15-min `hubspot_periodic_sync`) lands on a hardened pool, not a saturating one. Shape analogous to Pre-Phase 4 Session A's ops-hardening session. After it lands: Phase 4 Day 2 — coordinator + intelligence baseline (`IntelligenceCoordinator.receiveSignal` + `getActivePatterns` real implementations + `coordinator_synthesis` job + periodic `hubspot_periodic_sync` cron + worker retry/concurrency policies + telemetry dashboard), inheriting Session B's surfaces registry + admission engine + scoring prompt. See **`## Forward map`** section for downstream sequence.
...
2. **Pre-Phase-4-Day-2 — Ops/diagnostic: recurring pool-saturation root cause** (NEXT). Own session, ops-hardening shape analogous to Pre-Phase 4 Session A. Four-of-four sessions hit `EMAXCONN` (Phase 3 Day 4 Session B; Pre-Phase 4 Session A; Phase 4 Day 1 Session A.5; Phase 4 Day 1 Session B). Pre-Phase 4 Session A's circuit breaker handles symptom; root cause unidentified. Phase 4 Day 2 adds `coordinator_synthesis` job + 15-min `hubspot_periodic_sync` cron — net new baseline load on a system that's already saturating every session. Sequencing: this session lands FIRST so Phase 4 Day 2 inherits a hardened pool, not a saturating one. **Sequenced ahead of Phase 4 Day 2 per oversight (Session B closeout).** Likely scope shape: Vercel function inventory (per-function pool max + idle_timeout audit); local-script inventory (pool counts in test scripts + provisioning + smoke tests); cron-fired worker connection cost; `pg_stat_activity` snapshot when saturation reproduces; targeted mitigations (further per-service max reductions, route-level pool sharing, switching idle-heavy paths to DIRECT_URL where Vercel IPv6 readiness permits); refresh circuit breaker thresholds based on observed saturation cadence.
3. **Phase 4 — Intelligence layer** (~5 days; Day 1 splits A/A.5/B per execution decisions).
   ...
   - **Day 2** (NEXT) — `IntelligenceCoordinator.receiveSignal` + `getActivePatterns` real implementations + `coordinator_synthesis` job + periodic `hubspot_periodic_sync` cron + worker retry/concurrency policies + telemetry dashboard. Strongly recommended to investigate Decision 9's recurring pool saturation root cause BEFORE Day 2 lands the 15-min sync cron.
   - Day 3-4 — observation clustering + cross-deal pattern detection.
   - Day 5 — intelligence dashboard UI (consumes admitted set + scores from `SurfaceAdmission.admit`).
...
- ~~**Pool-saturation root-cause investigation (Decision 9 from Session B kickoff).**~~ **PROMOTED to Pre-Phase-4-Day-2 own session (oversight, Session B closeout).** Sequenced as item 2 in the Recommended sequence ABOVE Phase 4 Day 2; Phase 4 Day 2 inherits a hardened pool rather than landing coordinator + 15-min sync on a saturating system. See Recommended sequence for scope shape.
...
| Recurring pool-saturation root cause investigation | **Pre-Phase-4-Day-2 own session** (Decision 9 promoted) | Four-of-four sessions hit `EMAXCONN`; circuit breaker handles symptom; root cause unidentified. Promoted to dedicated own session per oversight (Session B closeout) — sequenced ahead of Phase 4 Day 2 so Day 2 inherits a hardened pool. |
