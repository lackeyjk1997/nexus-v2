...
## Current state (as of 2026-04-27 — Pre-Phase 4 Session A complete · Phase 4 ahead)
...
- **Pre-Phase 4 Session A complete.** Three small surface edits closing the recurring EMAXCONN cost. Shared-pool max=5 (was 10) in `packages/shared/src/db/pool.ts` (cuts per-Vercel-container budget consumption at the 200-client transaction-pooler cap from 5% → 2.5% per the empirical Phase 3 evidence). Worker-route circuit breaker in `apps/web/src/app/api/jobs/worker/route.ts` — pre-claim `SELECT 1` ping; on EMAXCONN match returns `503` + `Retry-After: 30` + structured `{error: "pool_saturated", retryAfterSeconds: 30}` body + stderr telemetry JSON line `{"event": "worker_circuit_break", "reason": "pool_saturated", ...}`; non-EMAXCONN errors fall through to existing `claim_failed` 500. `configure-cron.ts` precedence inverted: `DATABASE_URL ?? DIRECT_URL` (matches Day 4 Session B 7-script pattern; lets the script work on dev-Macs without working IPv6 routing to Supabase direct host). All three changes verified clean: `pnpm typecheck` 4/4 PASS, `pnpm build` 13 routes clean, `configure-cron` live exercise via pooler URL succeeded (`schedule="10 seconds", active=true`) — production state restored. Pool-saturation cascade paused at session start via the new `pool-session.mts` operational fallback shipped Day 4 Session B (cron paused via session pooler bypass to allow transaction pool to drain); session ends with cron rescheduled via the new pooler-URL configure-cron path. Total $0 live Claude / HubSpot / Voyage.
- **Next milestone:** **Phase 4 Day 1 — Intelligence layer kickoff.** `deal_events.event_context SET NOT NULL` migration per §2.16.1 decision 2 (all Phase 3-era writers populate it as of Day 4 Session B; flip is now safe). Applicability DSL + shared evaluator service (foundation review C2). Surfaces registry TS module + admission engine + scoring pass + dismissal/feedback wiring per §1.16-§1.17. `applicability_rejections` diagnostic table. See **`## Forward map`** section for the full sequence (Phase 4 Days 1-5 → Phase 5 Days 1-5 → Phase 6 with Phase 2 Day 4 Sessions C+D folded in) + parked items by phase + productization scope + honest assessment.
