...
- **Phase / Day completed:** Phase 2 Day 3 — deal detail + MEDDPICC edit + 4 contact-side adapter promotions.
- **Latest commit on `main` (nexus-v2):** `e0ef9b2 feat(phase-2-day-3): deal detail + MEDDPICC edit + contact stub promotions`.
...
- **Vercel production:** Deploying on `e0ef9b2`. `/pipeline/[dealId]` new route (4.1 kB first-load); kanban + table rows click-through to detail. 13 routes total (added `/pipeline/[dealId]` + permanent `/api/dev-login`).
- **Live HubSpot portal state (`245978261`):** pipeline `2215843570` + 9 stages unchanged; MedVista deal `321972856545` unchanged; Michael Chen (Chief of Surgery, `michael.chen@medvista-demo.example.com`) visible as the deal's only contact via `listDealContacts`. 38 `nexus_*` custom properties + 18 webhook subscriptions unchanged.
- **Live Supabase DB:** migration `0004_shallow_kid_colt.sql` applied. `meddpicc_scores` row exists for deal `321972856545` — metrics=8, champion=7, identify_pain=6, overall=7, evidence JSONB populated. Pattern D RLS verified end-to-end via `pnpm --filter @nexus/db tsx src/scripts/test-rls-meddpicc.ts`.
- **Next day scheduled:** Phase 2 Day 4 — stakeholder management UI (add/remove contacts, assign roles via `deal_contact_roles` + `setContactRoleOnDeal` stub promotion), kanban DnD stage change (`@dnd-kit/core`), dropdown stage change on detail + table row, Close Won / Close Lost outcome stubs, kanban filter chips (deferred from Day 3), DealCard hover-lift revisit (deferred from Day 3), deal-summary editability (Day 3 shipped read-only). Does not start until Jeff green-lights.
...
**MeddpiccService — the first purely-Nexus service.** `packages/shared/src/services/meddpicc.ts`. Class wraps `postgres.js` directly (matches `HubSpotAdapter`'s instance/close pattern; avoids introducing `@nexus/db` as a cross-package dep into `@nexus/shared`). Methods:
...
- **Kanban filter chips** (deferred from Day 3) — stage multi-select + vertical filter.
- **`DealCard` hover lift revisit** (deferred from Day 3) — now that click-through exists, re-evaluate whether the second shadow reads muddy; may land as `shadow-md` on hover + 1px translate, or stay as the current border-only transition.
- **Deal summary edit UI** — Day 3 shipped read-only `DealSummarySection`. Adds inline edit for vertical/product/lead source/competitor + company attributes; consumes `updateDeal` (already live) and `updateCompany` (promote from stub if not already live).
- Promote remaining adapter CRUD stubs as features require (`upsertContact`, `upsertCompany`, `updateCompany`, `deleteContact`, `deleteCompany`, `deleteDeal`).
- Revisit `/pipeline/:dealId` prefetch cost (see Pre-landing parked item) once the pipeline fills with many rows.
...
- **`MeddpiccService` is the pattern template for future Nexus-only services.** Phase 3's `SignalService`, `ObservationService`, `DealIntelligenceService`, etc., should mirror the shape: class in `packages/shared/src/services/`, `postgres.js` directly (not Drizzle — consistent with `HubSpotAdapter` and keeps `@nexus/shared` free of a `@nexus/db` dep), `{databaseUrl, sql?}` options for test-injection, `close()` method, request-scoped factory at `apps/web/src/lib/<name>.ts` mirroring `createHubSpotAdapter`. Auth gate at the caller (Server Action or route handler) using `createSupabaseServerClient().auth.getUser()` before touching the service. This note solidifies the pattern in one place until a second or third service lands and justifies a guardrail.
