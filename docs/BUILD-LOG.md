...
## Current state (as of 2026-04-28 — Pre-Phase-4-Day-2 complete · Phase 4 Day 2 ahead)
...
- **Next milestone:** **Phase 4 Day 2 — coordinator + intelligence baseline.** Inherits a leak-fixed worker route (Mitigation 1) + diagnostic ops tooling (`pool-snapshot.mts`) for empirical leak-fix verification at the 5-min + 30-min post-deploy windows. Day 2 scope: `IntelligenceCoordinator.receiveSignal` + `getActivePatterns` real implementations + `coordinator_synthesis` job + periodic `hubspot_periodic_sync` cron (15 min) + worker retry/concurrency policies + telemetry dashboard. Inherits Session B's surfaces registry + admission engine + scoring prompt. See **`## Forward map`** section for downstream sequence.
...
3. **Phase 4 — Intelligence layer** (~5 days; Day 1 splits A/A.5/B per execution decisions).
   ...
   - **Day 2** (NEXT) — `IntelligenceCoordinator.receiveSignal` + `getActivePatterns` real implementations + `coordinator_synthesis` job + periodic `hubspot_periodic_sync` cron + worker retry/concurrency policies + telemetry dashboard. Three explicit Pre-Phase-4-Day-2 carry-forwards (oversight closeout): (a) **load math uses 45-connection working headroom, NOT 200-cap** — synthetic harness showed ~155 platform-baseline consumption invisible from code, leaving ~45 application headroom; Day 2's coordinator + sync additions operate inside a tighter margin than the original 200-cap framing implied; (b) **`pool-snapshot.mts` is an EXPLICIT verification gate at 5-min + 30-min post-deploy**, not just inherited tooling — `candidate_leaks_60s_plus_excl_supavisor_recycle` must stay at 0 at both windows; (c) **four-of-four EMAXCONN pattern is structurally closed** — recurrence in Day 2 is a NEW pattern warranting its own investigation, not a return of the resolved one. See Pre-Phase-4-Day-2 closeout's "Parked items added" for full criteria.
...
- ~~**Pool-saturation root-cause investigation (Decision 9 from Session B kickoff).**~~ ✓ RESOLVED Pre-Phase-4-Day-2 (Path A conclusive). Hypothesis 1 (`createDb` per-invocation pool with `postgres.js` default `idle_timeout: 0`) named as root cause; worker route + `createDb` defaults fixed; `pool-snapshot.mts` durable diagnostic landed. Empirical verification window inherits to Phase 4 Day 2's first 30 min post-deploy.
