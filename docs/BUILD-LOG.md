...
- **Next milestone:** **Phase 4 Day 2 — coordinator + intelligence baseline.** Inherits the surfaces registry (admission thresholds for `intelligence_dashboard_patterns`), admission engine (consumed by Phase 4 Day 5 dashboard UI + Phase 5+ surfaces), and scoring prompt (per-candidate fanout discipline). Lands `IntelligenceCoordinator.receiveSignal` + `getActivePatterns` real implementations + `coordinator_synthesis` job + periodic `hubspot_periodic_sync` cron + worker retry/concurrency policies + telemetry dashboard. **Strongly recommended** to investigate the recurring pool saturation root cause (Decision 9 parked item) BEFORE Day 2 ships — Day 2 adds 15-min `hubspot_periodic_sync` baseline load on top of the already-elevated pool pressure. See **`## Forward map`** section for downstream sequence.
...
2. **Phase 4 — Intelligence layer** (~5 days; Day 1 splits A/A.5/B per execution decisions).
...
- **Pool-saturation root-cause investigation (Decision 9 from Session B kickoff).** Three of last four sessions hit `EMAXCONN`; Pre-Phase 4 Session A's circuit breaker handles the symptom but root cause unidentified. Investigate BEFORE Phase 4 Day 2 ships coordinator + 15-min `hubspot_periodic_sync` cron — both add baseline pool pressure on top of an already-elevated baseline.
...
- **Score-insight prompt grounding-discipline review.** Phase 4 Day 1 Session B's synthetic-path live exercise observed Claude inventing credible-sounding grounding details ("PHI residency," "CFO veto >$300K") not in the synthetic synthesis text. Score itself was in calibrated range so Session B's contract held, but if the pattern persists in Phase 4 Day 2's coordinator-driven pattern admissions, tighten `09-score-insight.md` (move "do not invent" earlier; add a worked-bad-example for invented stakeholders) + bump prompt version 1.0.0 → 1.1.0.
...
| Recurring pool-saturation root cause investigation | Pre-Phase-4-Day-2 (Decision 9) | Three of four sessions hit `EMAXCONN`; circuit breaker handles symptom; root cause unidentified. Phase 4 Day 2 adds 15-min `hubspot_periodic_sync` baseline load — investigate before Day 2 ships. |
...
| `09-score-insight` grounding-discipline review | Phase 5 Day 1 (or earlier if Phase 4 Day 2 surfaces it) | Live Session B exercise observed Claude inventing grounding details not in input; tighten prompt + bump 1.0.0 → 1.1.0 if pattern persists |
...
- **Score-insight prompt grounding-discipline watch (operational).** Synthetic-path live exercise confirmed Claude invented credible-sounding grounding details ("PHI residency," "CFO veto >$300K") not in the synthetic synthesis text. Score itself was in range; not a Session B blocker. If Phase 4 Day 2's coordinator-driven pattern admissions show the same pattern, tighten `09-score-insight.md` (move "do not invent" earlier; add a worked-bad-example for invented stakeholders) + bump prompt version 1.0.0 → 1.1.0.
