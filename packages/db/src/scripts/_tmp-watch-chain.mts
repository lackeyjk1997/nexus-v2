  ...
  for (;;) {
    const jobs = await sql`
      SELECT type, status, attempts, left(coalesce(error,''),160) AS err, created_at
        FROM jobs WHERE type IN ('granola_ingest','deal_fitness') AND created_at > now() - interval '20 minutes'
       ORDER BY created_at`;
    ...
    if (jobs.some((j)=>j.status==="failed" && j.attempts >= 3)) { console.log("JOB EXHAUSTED RETRIES"); break; }
