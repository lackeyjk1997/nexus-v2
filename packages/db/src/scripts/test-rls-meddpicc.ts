/**
 * Pattern D RLS verification for `meddpicc_scores` (Phase 2 Day 3).
 *
 * Policy shape (from 0001_auth_fk_and_rls.sql line 261):
 ...
 *
 * This test proves the property end-to-end:
 *   1. Sarah (authed anon client) INSERT → DENIED
 *   2. Sarah (authed anon client) UPDATE → DENIED
 *   3. Sarah (authed anon client) SELECT → allowed (read-all)
 *   4. Service-role UPSERT → allowed (bypasses RLS)
...
// Throwaway deal id — zero-pad so it's obviously a test fixture.
const TEST_DEAL_ID = "rls-test-000000-meddpicc";
...
async function main() {
  ...
  await admin.from("meddpicc_scores").delete().eq("hubspot_deal_id", TEST_DEAL_ID);
  ...
  const sarahInsert = await sarah.from("meddpicc_scores").insert({
  ...
  console.log(`[2/6] Sarah authed anon UPDATE on a row that doesn't exist (expect DENY or 0 rows via policy filter)…`);
  const sarahUpdate = await sarah.from("meddpicc_scores").update({
    metrics_score: 9,
  ...
  console.log(`      result: error=${sarahUpdate.error?.code ?? "none"} status=${sarahUpdate.status}`);

  console.log(`[3/6] Service-role UPSERT (expect SUCCESS — bypasses RLS)…`);
  const adminInsert = await admin.from("meddpicc_scores").upsert({
    hubspot_deal_id: TEST_DEAL_ID,
    metrics_score: 5,
    champion_score: 8,
    overall_score: 7,
  ...
  if (adminInsert.error) {
    throw new Error(`FAIL — service-role upsert errored: ${adminInsert.error.message}`);
  }
  console.log(`      OK`);
  ...
  const sarahSelect = await sarah
    ...
    .select("hubspot_deal_id, metrics_score, champion_score")
    .eq("hubspot_deal_id", TEST_DEAL_ID);
  if (sarahSelect.error || !sarahSelect.data?.length) {
    throw new Error(`FAIL — authed SELECT returned no row: ${JSON.stringify(sarahSelect.error ?? sarahSelect.data)}`);
  }
  console.log(`      OK — row returned: ${JSON.stringify(sarahSelect.data[0])}`);

  console.log(`[5/6] Marcus (different authed user) SELECT (expect SUCCESS — pattern D is read-all)…`);
  const marcus = await signInAs(marcusEmail);
  const marcusSelect = await marcus
    .from("meddpicc_scores")
    .select("hubspot_deal_id, metrics_score")
    .eq("hubspot_deal_id", TEST_DEAL_ID);
  if (marcusSelect.error || !marcusSelect.data?.length) {
    throw new Error(`FAIL — Marcus SELECT blocked: ${JSON.stringify(marcusSelect.error ?? marcusSelect.data)}`);
  }
  console.log(`      OK — Marcus can read the row Sarah/admin wrote (read-all semantics correct)`);
  ...
  console.log("");
  console.log("Pattern D RLS on meddpicc_scores: VERIFIED.");
