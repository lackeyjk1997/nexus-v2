...
- `${meddpiccBlock}: string` — pre-formatted by `DealIntelligence.formatMeddpiccForPrompt(dealId)`. Each of 8 dimensions on its own line: `- {dimension}: {evidence_text} (score: {n}, confidence: {n}%, last_updated: {iso_date})` or `- {dimension}: not yet captured`. (Drift-fix: was "7 dimensions" pre-§2.13.1 MEDDPICC 8-dim canonical; Phase 3 Day 3 Session A corrected in place at v1.1.0 — descriptive drift against the locked canonical, no behavioral change.)
